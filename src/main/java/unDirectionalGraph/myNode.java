...
public class myNode {
    ...
    public ArrayList<Pair<myNode, Double>> getNeighbor(String property_type) {
        ...
        Iterable<Relationship> rels = this.current.getRelationships(Line.Linked, Direction.BOTH);
        Iterator<Relationship> rel_Iter = rels.iterator();
        while (rel_Iter.hasNext()) {
            Relationship rel = rel_Iter.next();
            Node rel_start_Node = rel.getStartNode();
            Node rel_end_Node = rel.getEndNode();
            //Next node the node that is different current node
            Node nextNode = rel_end_Node.getId()==this.current.getId()?rel_start_Node:rel_end_Node;
