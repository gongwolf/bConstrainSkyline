...
public class SplitGraph {
    ...
    public void readFile() {
        String bs_path = "/home/gqxwolf/mydata/projectData/testGraph_real_50_int/data";
    ...
    public void mappingNode() {
        TreeMap<Integer, Integer> sortedNodeList = new TreeMap<>(this.node_city_map); //original node id --> city_id

        TreeMap<Integer, Integer> LA_Map = new TreeMap<>(); //original node id --> new id
        TreeMap<Integer, Integer> SF_Map = new TreeMap<>(); //original node id --> new id
        TreeMap<Integer, Integer> NY_Map = new TreeMap<>(); //original node id --> new id

        for (Map.Entry<Integer, Integer> e : sortedNodeList.entrySet()) {
            switch (e.getValue()) {
                case city_sf:
                    SF_Map.put(e.getKey(), SF_Map.size()); //new id is generated by checking the size of the map
                    break;
                case city_la:
    ...
    private void mappingEdges(TreeMap<Integer, Integer> node_Map, String city_name) {
        ...
        try {
            ...
            while (((readLine = b.readLine()) != null)) {
                ...
                //if the start node and the end node are in the same city
                if (node_Map.containsKey(sid) && node_Map.containsKey(did)) {
                    //read origin value of the costs (distance in meters, normal distribution)
...
//                    costs[2] = Double.parseDouble(readLine.split(" ")[4]);
                    ...
                    //change the value of the costs (all follow the uniform distribution)
                    costs[0] = randomFloatInRange(1,20);
                    costs[1] = randomFloatInRange(1,20);
                    costs[2] = randomFloatInRange(1,20);


//                    change the value of the costs (distance, other costs follow the uniform distribution)
//                    costs[0] = Double.parseDouble(readLine.split(" ")[2]);
//                    costs[1] = randomFloatInRange(1, 20);
//                    costs[2] = randomFloatInRange(1, 20);
