...
public class myshortestPathUseNodeModified {
    ...
    public myshortestPathUseNodeModified(GraphDatabaseService graphdb) {
    ...
    public void getSkylinePath(Node source, Node destination) {
        ...
        try (Transaction tx = this.graphdb.beginTx()) {
            ...
            //System.out.println(iniPath.propertiesName.get(0));
            //System.out.println(iniPath.propertiesName.get(1));
            //System.out.println(iniPath.propertiesName.get(2));
            //for (path p : this.skylinPaths) {
            //    System.out.println(printCosts(p.getCosts()));
            //}
            ////System.out.println(printCosts(this.iniLowerBound));
            //System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");

            myNode start = new myNode(source, source);
            start.setLowerBound(iniLowerBound);
            mqueue.add(start);
            tx.success();
        }
        ...
        sb.append(avg_slr_inNode / this.processedNodeList.size());
    ...
    private boolean needToBeExpanded(path p, Node destination) {
        ...
        if (!myEndNode.processed_lower_bound) {
            ...
            myEndNode.setLowerBound(tmplCosts);
            this.processedNodeList.put(endNode_id, myEndNode);
            myEndNode.processed_lower_bound = true;
            if (tmplCosts[0] == -1) {
                return false;
            }
        } else {
